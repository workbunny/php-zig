...
这是有意与 PHP 内部函数不同的一处，也是责任边界里最容易被误解的一格，故单独成节。

> **术语约定**：**cast 语义** = 我们的取值 API（等价 PHP 里的 `(int)$v` / `(string)$v`）；
> **ZPP 弱模式** = 原生内部函数的参数解析。本文不使用「弱转换」一词（它常被同时用来指这两者）。
...
> **提示**：以参数语义衡量，这套取值是**宽松**的 —— 传错类型不报错，`toLong()` 会把
> `object` 变成 `1`（附一条 `Warning`）。要严格须自己拿：
>
> - 声明 `Args`/`ArgTypes`（Reflection / IDE / 文档 / ZEND_DEBUG 断言），并在 handler 内
>   显式校验（`isLong()`/`isString()`/`asString()` + `Throw`）—— 框架不替你拦，
>   因为 Release 构建下引擎也不拦（见 §二 第 3、9 条）。
> - 取值 API 的定位是**永不返回垃圾值的 cast**，不是**参数校验器**。
